...
class MyClient(discord.Client):
    async def on_ready(self):
        await self.change_presence(activity=discord.Game(name="Free fire de tralha"))
        ...
        for mensagem in event.content.split():
            if mensagem in palavras:
                if event.content.find('gado') != -1 or event.content.find('gados') != -1:
                    await event.channel.send(f'{event.author.mention} escreveu: {event.content.replace("gados", "gostosos").replace("gado", "gostoso")}')
                await event.delete()
